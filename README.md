...
Форк Dimidrol добавляет операционный слой для ежедневной работы с vault:
...
Если upstream-патч уже был cherry-pick'нут, используйте обычный merge или `-s ours` только осознанно, чтобы отметить upstream commit как merged без перезаписи fork-specific изменений.
